    ...
    <AuthGuard>
      ...
      <main className="container mx-auto px-4 py-12 max-w-7xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          {[
            { label: "Overall Score", value: assessment?.overallScore, color: "text-primary" },
            { label: "Pass Rate", value: `${kpis.overallPassRate}%`, color: kpis.overallPassRate >= 80 ? "text-emerald-400" : "text-destructive" },
            { label: "Critical Issues", value: kpis.criticalCount, color: kpis.criticalCount > 0 ? "text-destructive" : "text-emerald-400" },
            { label: "WCAG A", value: kpis.totalA, color: "text-orange-400" },
            { label: "WCAG AA", value: kpis.totalAA, color: "text-yellow-400" }
          ].map((kpi, idx) => (
            <Card key={idx} className="glass-morphism p-4 text-center">
              <p className="text-[10px] font-bold uppercase text-muted-foreground mb-1">{kpi.label}</p>
